  ...
  const handleSubmit = () => {
    const newErrors = {
      ...
      arrivalTime: arrivalTime
        ? ""
        : "Arrival time is required bago mag submit",
    ...
    <div className="container">
      ...
      <div className={styles.main}>
        <div className={styles.input_field}>
          ...
          />
          {errors.arrivalTime && (
            <p style={{ color: "red" }}>{errors.arrivalTime}</p>
          )}
          Time Out:
          <input
            type="time"
        ...
        <div className={styles.trainees_m} style={{ marginTop: "1rem" }}>
          ...
          {filteredData.length > 0 ? (
            filteredData.map((item, index) => (
              <div
                className={styles.trainees}
                key={index}
                style={{ marginBottom: "1rem" }}
              >
                <div className={styles.details}>
                  <pre>
                    <span className={styles.details_b}>Name:</span> {item.name}
                    <br />
                    <span className={styles.details_b}>Date:</span> {item.date}
                    <br />
                    <span className={styles.details_b}>Time In:</span>{" "}
                    {item.arrivalTime}
                    <br />
                    <span className={styles.details_b}>Time Out:</span>{" "}
                    {item.timeOut}
                  </pre>
                </div>
